...
export async function getKnowledgeRadarData(userId: number) {
  ...
  return subjectMastery.map((s: any): { subject: string; mastery: number; totalPoints: number } => ({
    subject: s.subject,
...
export async function getKnowledgeHeatmapData(userId: number, subject: string) {
  ...
  return heatmapData.map((d: any): { knowledgePointId: number; name: string; grade: string; masteryLevel: number; errorCount: number } => ({
